...
function H2({ children, style = {} }: { children: React.ReactNode; style?: React.CSSProperties }) {
  return <h2 style={{ fontFamily: "'Montserrat', 'Space Grotesk', sans-serif", fontWeight: 700, fontSize: 'clamp(1.6rem, 3vw, 2.8rem)', letterSpacing: '-0.02em', lineHeight: 1.1, margin: 0, color: G900, ...style }}>{children}</h2>;
}

function H3({ children, style = {} }: { children: React.ReactNode; style?: React.CSSProperties }) {
  return <h3 style={{ fontFamily: "'Montserrat', 'Space Grotesk', sans-serif", fontWeight: 700, fontSize: 'clamp(1.1rem, 2vw, 1.5rem)', letterSpacing: '-0.015em', lineHeight: 1.25, margin: 0, color: G900, ...style }}>{children}</h3>;
}

function Body({ children, muted = false, style = {} }: { children: React.ReactNode; muted?: boolean; style?: React.CSSProperties }) {
  return <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 15, lineHeight: 1.75, color: muted ? G400 : G600, margin: 0, fontWeight: 400, ...style }}>{children}</p>;
}

function GlassCard({ children, style = {} }: { children: React.ReactNode; style?: React.CSSProperties }) {
  return (
    <div className="glass-card" style={{ background: WHITE, border: `1px solid ${G100}`, borderRadius: 16, boxShadow: '0 2px 12px rgba(0,0,0,0.06)', ...style }}>
      {children}
    </div>
  );
}
...
export default function Sobre() {
  return (
    <main style={{ background: BG, color: G900, fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <section style={{ position: 'relative', minHeight: '60vh', display: 'flex', alignItems: 'center', overflow: 'hidden', paddingTop: 100 }}>
        ...
        <div style={{ position: 'relative', zIndex: 10, maxWidth: 1280, margin: '0 auto', padding: 'clamp(16px,4vw,48px) clamp(16px,4vw,24px) clamp(48px,8vw,80px)', width: '100%' }}>
          <motion.div initial="hidden" animate="visible" variants={stagger} style={{ maxWidth: 760 }}>
            ...
            <motion.div variants={staggerItem}>
              <Body style={{ fontSize: 17, lineHeight: 1.8, maxWidth: 580, marginBottom: 36, color: 'rgba(255,255,255,0.75)' }}>
      ...
      {/* ── KEY STATS ────────────────────────────────────────────── */}
      <section style={{ padding: '0 24px 72px', position: 'relative', background: OFF }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger}
            style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16 }}>
            {KEY_STATS.slice(0, 4).map((s) => (
              <motion.div key={s.label} variants={staggerItem}>
                <GlassCard style={{ padding: '24px 20px', textAlign: 'center', background: OFF, border: `1px solid ${G100}` }}>
                  <div style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 'clamp(1.8rem, 3vw, 2.4rem)', fontWeight: 700, color: LIME_DIM, letterSpacing: '-0.02em', marginBottom: 6 }}>{s.prefix}{s.value}{s.suffix}</div>
                  <Body muted style={{ fontSize: 12, color: G400 }}>{s.label}</Body>
                </GlassCard>
              </motion.div>
            ))}
          </motion.div>
                  ...
                  <ul style={{ listStyle: 'none', padding: 0, margin: '0 0 20px', display: 'flex', flexDirection: 'column', gap: 7 }}>
                    {s.items.map((item) => (
                      <li key={item} style={{ display: 'flex', gap: 8, fontFamily: "'DM Sans', sans-serif", fontSize: 12, color: G600 }}>
                        <span style={{ color: LIME_DIM, flexShrink: 0 }}>✓</span>{item}
                      </li>
                    ))}
                  </ul>
      ...
      {/* ── AWARDS ───────────────────────────────────────────────── */}
      <section style={{ padding: '80px 24px', position: 'relative', background: OFF }}>
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          <Reveal style={{ textAlign: 'center', marginBottom: 48 }}>
            <EyebrowLabel>Reconhecimento Global</EyebrowLabel>
            <H2 style={{ color: G900 }}>Prêmios que Falam por Si.</H2>
          </Reveal>
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-8%' }} variants={stagger}
            ...
            {AWARDS.map((award) => (
              <motion.div key={award.title} variants={staggerItem}>
                  <GlassCard style={{ padding: '20px 22px', display: 'flex', gap: 14, alignItems: 'flex-start', background: award.highlight ? 'rgba(168,196,30,0.06)' : WHITE, border: award.highlight ? `1px solid ${LIME_DIM}` : `1px solid ${G100}` }}>
                    <div style={{ flexShrink: 0, padding: 8, borderRadius: 8, background: award.highlight ? 'rgba(168,196,30,0.12)' : OFF }}>
                    <Award size={16} style={{ color: award.highlight ? LIME_DIM : G200 }} />
                  </div>
                  <div>
                    <div style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 13, fontWeight: 700, color: G900, marginBottom: 4, lineHeight: 1.4 }}>{award.title}</div>
                    <Mono color={G400} size={11}>{award.org} · {award.year}</Mono>
                  </div>
                </GlassCard>
              </motion.div>
            ))}
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
            ...
            <Reveal>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 12, marginBottom: 24 }}>
                {[['+2M', 'Usuários ativos'], ['+200', 'Modelos de IA'], ['200k', 'Janela de contexto'], ['#6', 'Ranking G2 2024']].map(([v, l]) => (
                  <GlassCard key={l} style={{ padding: '16px 14px', textAlign: 'center', background: OFF, border: `1px solid ${G100}` }}>
                    <div style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '1.4rem', fontWeight: 700, color: LIME_DIM, marginBottom: 4 }}>{v}</div>
                    <Body muted style={{ fontSize: 11, color: G400 }}>{l}</Body>
                  </GlassCard>
                ))}
              </div>
          ...
          <Reveal style={{ marginBottom: 48 }}>
            ...
            <div style={{ maxWidth: 560, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 10 }}>
              {TESS_RANKING.map((r) => (
                <GlassCard key={r.rank} style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '14px 20px', background: r.highlight ? 'rgba(168,196,30,0.05)' : WHITE, border: r.highlight ? `1px solid ${LIME_DIM}` : `1px solid ${G100}` }}>
                  <div style={{ fontFamily: "'DM Sans', sans-serif", fontWeight: 800, fontSize: '1.4rem', width: 56, flexShrink: 0, textAlign: 'center', color: r.highlight ? LIME_DIM : G200 }}>{r.rank}</div>
                  <div style={{ fontFamily: "'DM Sans', sans-serif", fontWeight: 600, fontSize: 14, color: r.highlight ? G900 : G400 }}>{r.name}</div>
                  {r.highlight && <div style={{ marginLeft: 'auto', padding: '3px 10px', borderRadius: 99, background: G900, fontFamily: "'DM Sans', sans-serif", fontSize: 10, fontWeight: 700, color: LIME, letterSpacing: '0.1em' }}>NOSSA PLATAFORMA</div>}
                </GlassCard>
              ))}
            </div>
          </Reveal>
              ...
              <div style={{ overflowX: 'auto', WebkitOverflowScrolling: 'touch' }}>
                ...
                <thead>
                  <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
                    <th style={{ textAlign: 'left', padding: 'clamp(8px,2vw,14px) clamp(10px,2vw,20px)', fontFamily: "'DM Sans', sans-serif", fontSize: 12, color: 'rgba(136,146,164,0.5)', fontWeight: 500, whiteSpace: 'nowrap' }}>Área</th>
                    {[{ l: 'Tess AI v3', c: '#C8F135' }, { l: 'GPT-4 Turbo', c: 'rgba(136,146,164,0.5)' }, { l: 'Gemini Ultra', c: 'rgba(136,146,164,0.5)' }, { l: 'Llama 2', c: 'rgba(136,146,164,0.5)' }].map((h) => (
                      <th key={h.l} style={{ textAlign: 'center', padding: 'clamp(8px,2vw,14px) clamp(8px,1.5vw,16px)', fontFamily: "'DM Sans', sans-serif", fontSize: 12, color: h.c, fontWeight: 700, whiteSpace: 'nowrap' }}>{h.l}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {TESS_BENCHMARKS.map((b, i) => (
                    <tr key={b.area} style={{ background: i % 2 === 0 ? 'rgba(255,255,255,0.015)' : 'transparent', borderBottom: '1px solid #DDD7CF' }}>
                      <td style={{ padding: 'clamp(8px,2vw,12px) clamp(10px,2vw,20px)', fontFamily: "'DM Sans', sans-serif", fontSize: 13, color: G600, fontWeight: 600, whiteSpace: 'nowrap' }}>{b.area}</td>
                      <td style={{ padding: 'clamp(8px,2vw,12px) clamp(8px,1.5vw,16px)', textAlign: 'center', fontFamily: "'DM Sans', sans-serif", fontSize: 13, color: '#C8F135', fontWeight: 700, whiteSpace: 'nowrap' }}>{b.tess}</td>
                      <td style={{ padding: 'clamp(8px,2vw,12px) clamp(8px,1.5vw,16px)', textAlign: 'center', fontFamily: "'DM Sans', sans-serif", fontSize: 13, color: 'rgba(136,146,164,0.5)', whiteSpace: 'nowrap' }}>{b.gpt4}</td>
                      <td style={{ padding: 'clamp(8px,2vw,12px) clamp(8px,1.5vw,16px)', textAlign: 'center', fontFamily: "'DM Sans', sans-serif", fontSize: 13, color: 'rgba(136,146,164,0.5)', whiteSpace: 'nowrap' }}>{b.gemini}</td>
                      <td style={{ padding: 'clamp(8px,2vw,12px) clamp(8px,1.5vw,16px)', textAlign: 'center', fontFamily: "'DM Sans', sans-serif", fontSize: 13, color: 'rgba(136,146,164,0.5)', whiteSpace: 'nowrap' }}>{b.llama}</td>
                    </tr>
                  ))}
                </tbody>
                </table>
      ...
      {/* ── CLIENTES (logos) ─────────────────────────────────────── */}
        <section style={{ padding: '72px 24px', position: 'relative', overflow: 'hidden', background: WHITE }}>
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          <Reveal style={{ textAlign: 'center', marginBottom: 48 }}>
      ...
      {/* ── TIMELINE ─────────────────────────────────────────────── */}
      <section id="timeline" style={{ padding: '80px 24px', background: OFF }}>
        <div style={{ maxWidth: 860, margin: '0 auto' }}>
          <Reveal style={{ textAlign: 'center', marginBottom: 56 }}>
            <EyebrowLabel>Nossa História</EyebrowLabel>
            <H2 style={{ color: G900 }}>De 2011 a Hoje:<br /><span style={{ color: LIME_DIM }}>13 Anos de Liderança.</span></H2>
          </Reveal>
          <div style={{ position: 'relative' }}>
            <div style={{ position: 'absolute', left: 32, top: 0, bottom: 0, width: 1, background: `linear-gradient(to bottom, ${G200}, ${G400})` }} />
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-8%' }} variants={stagger} style={{ display: 'flex', flexDirection: 'column', gap: 32 }}>
              {TIMELINE.map((item, i) => (
                <motion.div key={item.year} variants={staggerItem} style={{ display: 'flex', gap: 32, paddingLeft: 32, position: 'relative' }}>
                  <div style={{ position: 'absolute', left: 0, top: 12, width: 64, height: 64, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'DM Sans', sans-serif", fontSize: 10, fontWeight: 800, flexShrink: 0, background: i === TIMELINE.length - 1 ? G900 : OFF, border: `2px solid ${i === TIMELINE.length - 1 ? LIME_DIM : G200}`, color: i === TIMELINE.length - 1 ? LIME : G400 }}>{item.year}</div>
                  <div style={{ paddingTop: 12, paddingBottom: 24, marginLeft: 40 }}>
                    <div style={{ fontFamily: "'Playfair Display', Georgia, serif", fontWeight: 700, fontSize: 16, color: G900, marginBottom: 8 }}>{item.title}</div>
      ...
      {/* ── CTA FINAL ────────────────────────────────────────────── */}
      <section style={{ padding: '96px 24px', textAlign: 'center', background: G900, position: 'relative', overflow: 'hidden' }}>
        <div className="data-grid" />
